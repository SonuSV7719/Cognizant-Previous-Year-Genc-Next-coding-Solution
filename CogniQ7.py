...
# solution my be wrong i just tested for 2 -3 test case


minBtn = int(input())
maxBtn = int(input())

skipBtnCnt = int(input())

skipBtn = set(int(input()) for _ in range(skipBtnCnt))

viewCnt = int(input())

viewNum = [int(input()) for _ in range(viewCnt)]

maxBtnReq = 0
prev = -1
cost1 = 2000000
for i in range(len(viewNum)):
    if viewNum[i] in skipBtn:
        continue

    if prev == viewNum[i]:
        continue
    if viewNum[i] == viewNum[i - 2] and i > 1:
        maxBtnReq += 1
        continue

    cntDig = len(str(viewNum[i]))
    if i > 0:
        cost1 = abs(viewNum[i] - viewNum[i - 1])
    cost2 = abs(viewNum[i] - maxBtn) + 1
    cost3 = abs(viewNum[i] - minBtn) + 1

    maxBtnReq += min(cntDig, cost1, cost2, cost3)
    # print(cntDig, cost1, cost2, cost3)
    prev = viewNum[i]
    # print(maxBtnReq)
